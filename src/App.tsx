...
export default function App() {
  ...
  const run = async () => {
    ...
    setPhase("reranking")
    try {
      const res = await getRecommendations({ query: jd, max_results: 10 })
      setResponse(res)
      setPhase("done")
    } catch (e) {
    ...
    <div style={{
      ...
      <nav style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          ...
          <a href="http://localhost:8000/docs" target="_blank" rel="noreferrer" style={{ fontSize: 10, color: "rgba(180,220,210,0.3)", letterSpacing: "0.08em", fontWeight: 600, transition: "color 0.2s" }}
      ...
      <div style={{ flex: 1, display: "grid", gridTemplateColumns: "1fr 1fr", overflow: "hidden", position: "relative" }}>
        ...
        {/* Token matrix background */}
        <div style={{ position: "absolute", inset: 0, zIndex: 0 }}>
          <TokenMatrix active={isRunning || phase === "done"} />
        </div>

        {/* Left fade — only covers left half so right side matrix is fully visible */}
        ...
        {/* ── LEFT PANEL ── */}
        <div style={{ position: "relative", zIndex: 2, padding: "0 52px", display: "flex", flexDirection: "column", justifyContent: "center", gap: 0 }}>
        ...
        <div ref={rightPanelRef} style={{ position: "relative", zIndex: 2, padding: "0 48px 0 24px", display: "flex", flexDirection: "column", justifyContent: "center", gap: 12 }}>
          ...
          {/* Results — scrollable within right panel */}
          {response && (
            <div style={{ animation: "fadeUp 0.4s ease both" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10, paddingLeft: 2 }}>
                <div>
                  ...
                  <span style={{ fontSize: 10, color: "rgba(180,220,210,0.3)", marginLeft: 10 }}>
                    {filteredResults.length} assessments
                  </span>
                </div>
          ...
          {isRunning && !response && (
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {[1, 2, 3].map(i => (
                <div key={i} style={{ height: 64, borderRadius: 10, background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.05)", animation: `pulse 1.5s ease ${i * 0.15}s infinite` }} />
              ))}
            </div>
          )}
        </div>
      ...
      <div style={{
        ...
        <span style={{ fontWeight: 700, fontSize: 11, letterSpacing: "-0.01em", color: "rgba(180,220,210,0.3)" }}>SHL Flow</span>
